                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {activeTab === 'assignedToMe' && task.status !== 'Completed' && (
                            <>
                              {task.status === 'To Do' && (
                                <button onClick={() => updateStatus(task.id, 'In Progress')} className="px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 transition-colors rounded-lg text-xs font-semibold">
                                  Start
                                </button>
                              )}
                              {task.status === 'In Progress' && (
                                <button onClick={() => updateStatus(task.id, 'To Do')} className="px-3 py-1.5 bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors rounded-lg text-xs font-semibold">
                                  Stop
                                </button>
                              )}
                              {(task.status === 'To Do' || task.status === 'In Progress') && (
                                <button onClick={() => updateStatus(task.id, 'Completed')} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors rounded-lg text-xs font-semibold flex items-center gap-1">
