...
async function uploadFile(filePath, collectionName) {
  ...
  const targetColPath = `userData/${companyId}/segments/General/crmData/${collectionName}/items`;
  ...
  const batchSize = 100; // Batch max is 500
...
async function run() {
  const files = [
    { path: path.join(process.cwd(), 'public', 'regular_leads_backup_2026-08-21 (2).json'), col: 'leads' },
    { path: path.join(process.cwd(), 'public', 'ad_leads_backup_2026-08-21.json'), col: 'adLeads' },
    { path: path.join(process.cwd(), 'public', 'distributors_backup_2026-08-21.json'), col: 'distributors' }
  ];
